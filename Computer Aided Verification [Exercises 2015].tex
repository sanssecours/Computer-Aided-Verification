...
\begin{enumerate}[label=(\alph*)]
    ...
    This is not the case, since there exist formulas where we can always generate larger counterexamples. Take for example the Kripke structure below. Here a valid counterexample for $s₀⊧FX¬p$ is the path $s₀s₁$ but also $s₀s₁s₀s₁$. We can always expand this counterexample by adding $s₀s₁$ at the end.

    \begin{center}
        \includegraphics[width=.4\textwidth]{Figures/Simple Kripke Structure.pdf}
    \end{center}
